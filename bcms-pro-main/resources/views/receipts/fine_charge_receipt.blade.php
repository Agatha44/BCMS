...
<table class="ctable" style="font-size: 9pt; margin-top: 25px;">
    ...
    <tr>
        ...
        <td colspan="3">{{ $receipt_data['charge_description'] ?? 'Fine Charge' }}</td>
    </tr>
    <tr>
        <th>Payment Type:</th>
        <td>Fine Charge Billing</td>
        <th>Charge Date:</th>
        <td>{{ $receipt_data['charge_date'] ? \Carbon\Carbon::parse($receipt_data['charge_date'])->format('d-M-Y') : '-' }}</td>
    </tr>
    <tr>
        <th>Payer Name:</th>
    ...
    <tr>
        <th>Plate Number:</th>
        <td>{{ $receipt_data['plate_number'] ?? '-' }}</td>
        <th>Driver Name:</th>
        <td>{{ $receipt_data['driver_name'] ?? '-' }}</td>
    </tr>
</table>
</body>
</html>
