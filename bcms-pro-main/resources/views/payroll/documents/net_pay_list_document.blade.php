@php
    ...
    $amountField = (string) ($amountField ?? 'net_pay');
...
<head>
    <meta charset="UTF-8">
    <title>{{ $documentTitle ?? 'Payroll Net Pay Sheet' }}</title>
    <style>
        @page { margin: 10mm; }
        body {
...
<div class="report">
    ...
    <div class="document-title">{{ $documentTitle ?? 'Payroll Net Pay Sheet' }}</div>
    ...
    <table class="info">
        ...
        @if(!empty($runInfo['bank_bucket_label']))
        <tr>
            <th>Salary Bank Batch</th>
            <td>{{ $runInfo['bank_bucket_label'] }}</td>
            <th>Total {{ $amountColumnLabel ?? 'Net Pay' }}</th>
            <td class="left total-value">{{ number_format((float) $totalAmount, 2) }}</td>
        </tr>
        @else
        <tr>
            <th></th>
    ...
    <table class="grid" autosize="1">
        ...
        <tr>
            ...
            <th>Bank</th>
            <th>Account No.</th>
            <th>{{ $amountColumnLabel ?? 'Net Pay' }}</th>
        </tr>
        </thead>
        <tbody>
        @forelse($employees as $index => $employee)
            ...
            <tr>
                ...
                <td class="center">{{ $employee->account_number ?? '' }}</td>
                <td class="right">{{ number_format((float) ($employee->{$amountField}), 2) }}</td>
            </tr>
        @empty
            <tr>
                <td colspan="6" class="center">No employees with net pay for this sheet.</td>
            </tr>
        @endforelse
        </tbody>
        <tfoot>
