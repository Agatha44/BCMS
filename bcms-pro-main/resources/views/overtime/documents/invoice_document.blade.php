...
<div class="invoice-info">
    <p><strong>From:</strong>Bridge In charge</p>
    ...
    <p><strong>Subject:</strong> Request for payment of overtime</p>
</div>
...
<table>
    ...
    <tr>
        ...
        <th style="width: 10%;" class="text-right">Salary</th>
        <th style="width: 10%;" class="text-right">Total Days</th>
        <th style="width: 15%;" class="text-right">Gross Pay (TZS)</th>
        <th style="width: 15%;" class="text-right">Payee (Tax) (TZS)</th>
        <th style="width: 15%;" class="text-right">Net Pay (TZS)</th>
    </tr>
    </thead>
    <tbody>
    @forelse($employees as $index => $employee)
        <tr>
            ...
            <td class="text-right">{{ number_format($employee->basicsalary ?? 0, 2) }}</td>
            <td class="text-right">{{ $employee->total_days ?? 0 }}</td>
            <td class="text-right">{{ number_format($employee->gross_pay ?? 0, 2) }}</td>
            <td class="text-right">{{ number_format($employee->tax ?? 0, 2) }}</td>
            <td class="text-right">{{ number_format($employee->net_pay ?? 0, 2) }}</td>
        </tr>
    @empty
        <tr>
    ...
    @php
        $totalGrossPay = collect($employees)->sum('gross_pay');
        $totalTax = collect($employees)->sum('tax');
    @endphp
    <tfoot>
    <tr class="total-row">
        <!-- {{-- Empty cells to keep alignment with the header columns --}} -->
        <td colspan="6" class="text-right"><strong>Totals:</strong></td>
        <!-- {{-- Total Gross Pay under "Gross Pay (TZS)" column --}} -->
        <td class="text-right"><strong>{{ number_format($totalGrossPay ?? 0, 2) }}</strong></td>
        <!-- {{-- Total Tax under "Payee (Tax) (TZS)" column --}} -->
        <td class="text-right"><strong>{{ number_format($totalTax ?? 0, 2) }}</strong></td>
        <!-- {{-- Total Net Pay / Batch Amount under "Net Pay (TZS)" column --}} -->
