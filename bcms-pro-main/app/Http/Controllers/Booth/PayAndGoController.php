...
class PayAndGoController extends ConfigurationController
{
    ...
    public function payAndGo(Request $request)
    {
        ...
        $cashlessVehicle = DB::table('vehicle as v')
            ->join('account_vehicle as av', 'av.vehicle_id', '=', 'v.id')
            ->join('account as a', 'a.account_no', '=', 'av.account_id')
            ->select([
                'v.*',
                'a.account_no',
                'a.status as account_status',
                'av.account_id as account_id',
                'av.status as account_vehicle_status',
            ])
...
//            return $processedVehicleBundlePassage;
            ...
            $processedCreditBalancePassage = $this->processCreditBalancePassage($request, $cashlessVehicle);
    ...
    private function processCreditBalancePassage(Request $request, $cashlessVehicle): array
    {

        if ($cashlessVehicle == null) {
            return ['status' => false, 'message' => 'Cashless vehicle not found or not active'];
        }

        $account = Account::where('account_no', $cashlessVehicle->account_id)->first();
        if ($account == null) {
        ...
        if ($account->account_balance >= $request->amount) {
            ...
            Account::where(['account_no'=>$cashlessVehicle->account_id])->update([
...
//            $account->save();
            ...
            // record transaction
            $transaction = TollTransaction::recordTollTransaction(false, 'CASHLESS', $request, $cashlessVehicle->id, $account->account_no);
            ...
            $sms_body = 'Makato ya Tozo ya kupita darajani' . "\n" .
                'Akaunti: ' . $cashlessVehicle->account_id . "\n" .
            ...
            return [
                ...
                'response' => [
                    'status' => 4,
                    'tag_reference' => $cashlessVehicle->rfid_tag_no,
                    'amount' => $request->amount,
                    'account' => $account,
                    'body_type' => $cashlessVehicle->body_type_id,
