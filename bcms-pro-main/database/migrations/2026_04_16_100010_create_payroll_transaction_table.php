...
return new class extends Migration
{
    ...
    {
        Schema::connection('bcmis2')->create('payroll_transaction', function (Blueprint $table) {
            $table->bigIncrements('id');

            // Link to payroll run (IMPORTANT for traceability)
            ...
            // Helpful employee snapshot fields for export/posting (denormalized)
            ...
            // Arrears: keep both naming variants (HRMA uses totalarrears)
            ...
            // Posting/export lifecycle (ERP/bank file)
            ...
            // Indexes
            $table->index(['national_id', 'payroll_year', 'payroll_month'], 'idx_payroll_tx_emp_period');
            $table->index(['payroll_run_id'], 'idx_payroll_tx_run');
            $table->index(['bank_id'], 'idx_payroll_tx_bank_id');
            $table->index(['erms_status'], 'idx_payroll_tx_erms_status');
            ...
            $table->unique(['payroll_run_id', 'national_id'], 'uniq_payroll_tx_run_emp');
        });
    }

    public function down(): void
    {
        Schema::connection('bcmis2')->dropIfExists('payroll_transaction');
    }
};
