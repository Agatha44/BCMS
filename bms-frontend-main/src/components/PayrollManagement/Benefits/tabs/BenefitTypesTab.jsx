...
import { formatNumber } from '../../../../common/utils/numberFormat.js';
import CollectionLoader from '../../../../pages/CollectionManagement/components/CollectionLoader.jsx';
import { payrollService } from '../../../../services/payrollService.js';
import AddBenefitTypeModal from '../AddBenefitTypeModal.jsx';
import BenefitTypeDetailsModal from '../BenefitTypeDetailsModal.jsx';

const BenefitTypesTab = ({ canEdit = false, active = false }) => {
  ...
  const refresh = useCallback(
    async (page = 1, pageSize = 15) => {
      setLoading(true);
      try {
        const response = await payrollService.getBenefitTypes({ page, per_page: pageSize });
  ...
  const toggleStatus = useCallback(
    async (record) => {
      const benefitTypeId = record?.benefit_type_id;
      if (!benefitTypeId) {
        message.error('Benefit type ID is missing.');
        return;
      }
      ...
      modal.confirm({
        title: `Are you sure you want to ${action} this benefit type?`,
        content: record?.benefit_name ? `"${record.benefit_name}" will be ${action}d.` : undefined,
        okText: `Yes, ${action}`,
        cancelText: 'Cancel',
        ...
        async onOk() {
          ...
          setRows((prev) =>
            prev.map((r) => {
              const rid = r?.benefit_type_id;
              if (rid === benefitTypeId) {
          ...
          const response = await payrollService.toggleBenefitTypeStatus(benefitTypeId, { is_active: currentActive ? 0 : 1 });
          if (!response?.success) {
  ...
  const columns = useMemo(
    () => [
      ...
      { title: 'Code', dataIndex: 'benefit_code', key: 'benefit_code', searchable: true, width: 110 },
      {
      ...
      },
      {
        ...
        width: 160,
        align: 'right',
        render: (v) => formatNumber(v, { minimumFractionDigits: 0, maximumFractionDigits: 2 }),
      },
      {
        title: 'Status',
        dataIndex: 'is_active',
...
BenefitTypesTab.propTypes = {
  canEdit: PropTypes.bool,
  active: PropTypes.bool,
};

export default BenefitTypesTab;
