...
import AddEmployeeBenefitModal from '../AddEmployeeBenefitModal.jsx';
import EmployeeBenefitDetailsModal from '../EmployeeBenefitDetailsModal.jsx';

const EmployeeBenefitsTab = ({ canEdit = false, active = false }) => {
  ...
  const refresh = useCallback(
    async (page = 1, pageSize = 15) => {
      setLoading(true);
      try {
        const response = await payrollService.getEmployeeBenefits({ page, per_page: pageSize });
  ...
  const toggleStatus = useCallback(
    async (record) => {
      ...
      if (!employeeBenefitId) {
        message.error('Employee benefit ID is missing.');
        return;
      }
      ...
      modal.confirm({
        title: `Are you sure you want to ${action} this employee benefit?`,
        ...
        async onOk() {
          setRows((prev) =>
            prev.map((r) => {
              const rid = r?.employee_benefit_id;
              if (rid === employeeBenefitId) {
          ...
          const response = await payrollService.toggleEmployeeBenefitStatus(employeeBenefitId, {
...
EmployeeBenefitsTab.propTypes = {
  canEdit: PropTypes.bool,
  active: PropTypes.bool,
};

export default EmployeeBenefitsTab;
