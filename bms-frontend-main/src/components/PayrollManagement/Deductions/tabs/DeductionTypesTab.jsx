...
import AddDeductionTypeModal from '../AddDeductionTypeModal.jsx';
import DeductionTypeDetailsModal from '../DeductionTypeDetailsModal.jsx';

const DeductionTypesTab = ({ canEdit = false, active = false }) => {
  ...
  const refresh = useCallback(
    async (page = 1, pageSize = 15) => {
      setLoading(true);
      try {
        const response = await payrollService.getDeductionTypes({ page, per_page: pageSize });
  ...
  const toggleStatus = useCallback(
    async (record) => {
      const deductionTypeId = record?.deduction_type_id;
      if (!deductionTypeId) {
        message.error('Deduction type ID is missing.');
        return;
      }
      ...
      modal.confirm({
        title: `Are you sure you want to ${action} this deduction type?`,
        ...
        async onOk() {
          ...
          setRows((prev) =>
            prev.map((r) => {
              const rid = r?.deduction_type_id;
              if (rid === deductionTypeId) {
          ...
          const response = await payrollService.toggleDeductionTypeStatus(deductionTypeId, { is_active: currentActive ? 0 : 1 });
          if (!response?.success) {
  ...
  const columns = useMemo(
    () => [
      ...
      { title: 'Code', dataIndex: 'deduction_code', key: 'deduction_code', searchable: true, width: 110 },
      {
