...
const mapOverloadToForm = (overload = {}) => ({
  first_name: overload.first_name || '',
  middle_name: overload.middle_name || '',
  surname: overload.surname || '',
  pyr_cell_num: overload.pyr_cell_num || '',
  pyr_email: overload.pyr_email || '',
  tin_number: overload.tin_number || '',
  bill_amount: overload.bill_amount ?? 0,
  ticket_num: overload.ticket_num || '',
  vehicle_num: overload.vehicle_num ? String(overload.vehicle_num).trim().toUpperCase() : '',
  bill_desc: overload.bill_desc || '',
});

export default function EditOverloadFineModal({ isOpen, overload, onClose, onUpdated }) {
  ...
  useEffect(() => {
    if (!isOpen || !overload) return;
    form.setFieldsValue(mapOverloadToForm(overload));
  }, [form, isOpen, overload]);

  const handleSubmit = async (values) => {
    if (!overload?.id) return;
    ...
    try {
      ...
      await overloadFineService.update(payload);
      onUpdated?.();
      closeModal();
    } catch (error) {
      await Swal.fire({
        icon: 'error',
        title: 'Failed',
        text: error?.message || 'Failed to update overload fine',
      });
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    ...
    >
      <BrandModalHeader title="Edit Overload Fine" onClose={closeModal} />
      <Form
        form={form}
        layout="vertical"
...
EditOverloadFineModal.propTypes = {
...
EditOverloadFineModal.defaultProps = {
  overload: null,
  onClose: undefined,
  onUpdated: undefined,
};
