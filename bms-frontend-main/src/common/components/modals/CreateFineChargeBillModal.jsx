...
export default function CreateFineChargeBillModal({ isOpen, onClose, onCreated }) {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const payload = {
        ...
        charge_date: values.charge_date.format('YYYY-MM-DD'),
        charge_description: values.charge_description.trim(),
        bill_amount: Number(values.bill_amount),
      };

      const resp = await apiService.createFineChargeBill(payload);
      if (!resp.success) {
        await Swal.fire({ icon: 'error', title: 'Failed', text: resp.message || 'Failed to create fine charge bill' });
        return;
      }
      ...
      >
        <div className="mb-4">
          ...
          <div className="grid grid-cols-1 gap-x-5 md:grid-cols-2">
            ...
            >
              <Input size="large" placeholder="Optional driver name" disabled={submitting} />
            </Form.Item>
            ...
            <Form.Item
              name="charge_date"
              label={<span className="text-sm font-medium" style={{ color: BRAND }}>Charge Date</span>}
            ...
            <Form.Item
              ...
              label={<span className="text-sm font-medium" style={{ color: BRAND }}>Charge Description</span>}
              rules={[{ required: true, message: 'Charge description is required' }]}
            >
              <Input.TextArea
                rows={3}
                placeholder="Describe the fine charge"
                disabled={submitting}
              />
            </Form.Item>
          </div>
...
CreateFineChargeBillModal.propTypes = {
