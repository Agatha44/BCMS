...
import {Ban, Copy, Receipt} from 'lucide-react';
...
const EVENT_BILL_PREFIX_FALLBACK = 'ECP';
...
const buildBillId = (bill) => {
    ...
    const prefix = String(bill?.source ?? '').trim() || EVENT_BILL_PREFIX_FALLBACK;
    return `${prefix}${id}`;
};
...
export default function EventBillDetailsModal({
    isOpen,
    bill = null,
    onClose,
    ...
    const anyLoading = cancelSubmitting || loadingReceipt;
    ...
    const summary = useMemo(() => {
        ...
        return {
            ...
            billId: bill.id,
            description: bill.event_description ?? bill.bill_desc,
            payer: cleanName(bill.payer_name ?? bill.cust_name ?? bill.customer ?? bill.customer_name),
            receiver: cleanName(bill.receiver_name),
            phone: bill.phone_number ?? bill.payer_phone,
            email: bill.email ?? bill.payer_email,
            receipt: bill.psp_receipt_num ?? bill.receipt_number ?? bill.receipt,
            eventDate: formatDateOnly(bill.event_date),
    ...
    const handleOpenReceipt = async () => {
        ...
        try {
            const resp = await apiService.getEventPaymentReceipt({id: bill.id});
    ...
    return (
        <>
            ...
            >
                <BrandModalHeader title="Event Bill" onClose={handleClose} />
                    ...
                    <>
                        <div className="max-h-[70vh] overflow-y-auto px-6 py-5">
                            ...
                            <div className="mb-5 grid grid-cols-1 gap-x-6 sm:grid-cols-2">
                                ...
                                <ReadOnlyField label="Receiver Name" value={summary.receiver} />
                            ...
                            <div className="mb-5 grid grid-cols-1 gap-x-6 sm:grid-cols-2">
                                <ReadOnlyField label="Event Date" value={summary.eventDate} />
                                <ReadOnlyField label="Bill Amount" value={summary.amount} mono />
                    ...
                    <div className="px-6 py-5">
                        <div className="mb-4 rounded-md border border-amber-200 bg-amber-50 px-4 py-3">
                            <p className="text-sm text-amber-900">
                                This will cancel the event bill. Continue?
                            </p>
                        </div>
                        <Form.Item
                            name="cancel_reason"
...
EventBillDetailsModal.propTypes = {
