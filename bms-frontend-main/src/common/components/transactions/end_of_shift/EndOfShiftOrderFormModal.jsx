...
import { buildEosBillOrderForm } from './eosBillOrderFormUtils.js';
  ...
  >
    ...
    >
      ...
      <div style={{ flex: 1, textAlign: 'center', padding: '0 12px' }}>
        <p style={{ margin: 0, fontSize: 13, fontWeight: 700 }}>THE UNITED REPUBLIC OF TANZANIA</p>
        <p style={{ margin: '4px 0 0', fontSize: 13, fontWeight: 700 }}>NATIONAL SOCIAL SECURITY FUND</p>
      </div>
      <img
        src={tanzaniaEmblem}
        alt="Coat of Arms"
    ...
    >
      ORDER FORM FOR ELECTRONIC FUNDS TRANSFER
    </h1>
...
const buildPdfFilename = (form, bill) => {
  ...
  return `shift-payment-order-form-${safe}.pdf`;
};

export default function EndOfShiftOrderFormModal({ open, bill, loading = false, onClose }) {
  ...
  const printedBy = useMemo(() => {
    ...
    return (
      user.full_name ||
      user.name ||
      [user.first_name, user.middle_name, user.surname].filter(Boolean).join(' ') ||
      user.username ||
      ''
    );
  }, [user]);

  const form = useMemo(() => (bill ? buildEosBillOrderForm(bill, printedBy) : null), [bill, printedBy]);
  ...
  const handlePrint = useCallback(() => {
    ...
    if (!win) return;
    win.document.write(
      `<!DOCTYPE html><html><head><title>Shift Payment Order Form</title><style>${printStyles}</style></head><body>${html}</body></html>`
    );
    win.document.close();
    win.focus();
    win.onload = () => {
  ...
  return (
    ...
    >
      <BrandModalHeader title="Shift Payment Order Form" onClose={onClose} />
...
EndOfShiftOrderFormModal.propTypes = {
...
EndOfShiftOrderFormModal.defaultProps = {
  bill: null,
  loading: false,
};
