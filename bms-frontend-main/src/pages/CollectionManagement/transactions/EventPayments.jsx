...
import CreateEventBillModal from '../../../common/components/modals/CreateEventBillModal.jsx';
import EventBillDetailsModal from '../../../common/components/transactions/events/EventBillDetailsModal.jsx';
...
export default function EventPayments() {
  ...
  const fetchBills = useCallback(async (page = filters.page, perPage = filters.per_page, search = filters.search) => {
    ...
    try {
      ...
      const resp = await apiService.getEventBills(payload);
      if (!resp.success) {
        setRows([]);
        setError(resp.message || 'Failed to load event bills');
        return;
      }
      ...
      setRows([]);
      setError('An error occurred while loading event bills');
  ...
  return (
    <>
      <div className="space-y-4">
        ...
        <div className="relative">
          ...
          <DataTable
            ...
            rowKey={(r, idx) => r.id ?? r.contr_num ?? r.control_display ?? `event-${idx}`}
            showSearch={true}
            showGlobalSearch={true}
            searchPlaceholder="Search event bills by customer, control, receipt..."
      ...
      <CreateEventBillModal
        isOpen={showCreateModal}
        onClose={() => setShowCreateModal(false)}
        onCreated={(resp) => {
          setBillSuccess({
            title: 'Event bill created',
      ...
      >
        ...
        <div className="px-6 py-6">
          <div className="mb-5 flex flex-col items-center text-center">
            ...
            <h3 className="mt-3 text-base font-semibold text-slate-900">Event bill created successfully</h3>
            {billSuccess?.message && (
