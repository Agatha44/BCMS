...
import CreateFineChargeBillModal from '../../../common/components/modals/CreateFineChargeBillModal.jsx';
import FineChargeBillDetailsModal from '../../../common/components/transactions/finecharges/FineChargeBillDetailsModal.jsx';
...
const normalizeRow = (row = {}) => {
  const customer = row.payer_name ?? row.customer_name ?? row.cust_name ?? '-';
  ...
  const amount = row.bill_amount ?? row.amount ?? row.amount_collected;
...
export default function FineChargeBilling() {
  ...
  const fetchBills = useCallback(async (page = filters.page, perPage = filters.per_page, search = filters.search) => {
    ...
    try {
      ...
      const resp = await apiService.getFineChargeBills(payload);
      if (!resp.success) {
        setRows([]);
        setError(resp.message || 'Failed to load fine charge bills');
        return;
      }
      ...
      setRows([]);
      setError('An error occurred while loading fine charge bills');
  ...
  return (
    <>
      <div className="space-y-4">
        ...
        <div className="relative">
          ...
          <DataTable
            ...
            rowKey={(r, idx) => r.id ?? r.contr_num ?? r.control_display ?? `fine-charge-${idx}`}
            showSearch={true}
            showGlobalSearch={true}
            searchPlaceholder="Search fine charge bills by customer, control, receipt..."
      ...
      <CreateFineChargeBillModal
        isOpen={showCreateModal}
        onClose={() => setShowCreateModal(false)}
        onCreated={(resp) => {
          setBillSuccess({
            title: 'Fine charge bill created',
      ...
      >
        ...
        <div className="px-6 py-6">
          <div className="mb-5 flex flex-col items-center text-center">
            ...
            <h3 className="mt-3 text-base font-semibold text-slate-900">Fine charge bill created successfully</h3>
            {billSuccess?.message && (
